/**
 * Channel list for question_approval (form/assignment) approval tab.
 * Uses question_approval socket/API types only.
 */

import React, { useMemo, useState, useCallback, useEffect } from 'react';
import { Typography, theme, Empty, Collapse, Tag } from 'antd';
import { UserOutlined, MessageOutlined, ClockCircleOutlined, CheckCircleOutlined, CloseCircleOutlined } from '@ant-design/icons';
import { Channel } from '../types';
import {
  type QuestionApprovalChannelRecord,
  transformQuestionApprovalChannelToChannel,
} from '../../../services/queueApi';

const { Panel } = Collapse;
const { Text } = Typography;

export interface QuestionApprovalChannelsListProps {
  records: QuestionApprovalChannelRecord[];
  onChannelSelect: (channel: Channel) => void;
  selectedChannelId?: string | null;
}

export const QuestionApprovalChannelsList: React.FC<QuestionApprovalChannelsListProps> = ({
  records,
  onChannelSelect,
  selectedChannelId,
}) => {
  ...
  const recordsWithChannels = useMemo(() => {
    ...
    return records.map((record, index) => {
      ...
      if (user && typeof user === 'object' && 'name' in user) {
        assigneeName = (user as { name?: string }).name || 'Unknown User';
      } else if (typeof user === 'string') {
        assigneeName = user;
      }
      const formName = record.assignment?.formTemplate?.name || 'Unknown Form';
      const channels: Channel[] = [];
      if (record.channels?.length) {
        record.channels.forEach((channelData) => {
          try {
            channels.push(transformQuestionApprovalChannelToChannel(channelData, record));
          } catch (error) {
            console.error('[QuestionApprovalChannelsList] Error transforming channel:', error, channelData);
          }
        });
      }
      return {
        ...
        formName,
        channels,
        panelKey: `${record.assigneeId}-${record.assignmentId}-${index}`,
      };
    });
  }, [records]);
  ...
  if (recordsWithChannels.length === 0) {
    return (
      <div style={{ padding: '24px', textAlign: 'center' }}>
        <Empty description="No form approval records" image={Empty.PRESENTED_IMAGE_SIMPLE} />
      </div>
    );
  }
