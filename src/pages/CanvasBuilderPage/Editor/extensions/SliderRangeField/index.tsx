...
import SliderComponent from './view';
import { genId } from '../../utils';

export const SliderNode = Node.create({
  name: 'sliderField',
  group: 'block',
  atom: false,
  draggable: true,
  ...
  addAttributes() {
    return {
      ...
      max: { default: 10 },
      step: { default: 1 },
      rangeMode: { default: false }, // single or range
      marks: { default: undefined }, // array or object
      value: { default: undefined },
      showTicks: { default: false },
      required: { default: false },
      approvalRequired: { default: false },
      displayValue: { default: 'tooltip' }, // 'tooltip' | 'above' | 'below' | 'none'
      displayStepValues: { default: 'minmax' }, // 'none' | 'minmax' | 'all'
      ...
      nodeGroupValues: {
        default: {} as Record<string, any>, // entityId -> value (number | [number, number])
        parseHTML: (element) => {
  ...
  parseHTML() {
    return [{ tag: 'div[data-type="slider-field"]' }];
  },

  renderHTML({ HTMLAttributes }) {
    return [
      'div',
      mergeAttributes(HTMLAttributes, { 'data-type': 'slider-field' }),
      0,
    ];
  },

  addNodeView() {
    return ReactNodeViewRenderer(SliderComponent);
  },
    ...
    return {
      insertSliderField:
        ...
        ({ commands }: { commands: CommandProps['commands'] }) => {
          const label = (attrs as Attributes)?.label ?? 'Slider';
          return commands.insertContent({
            ...
            attrs: {
              ...
              max: attrs?.max !== undefined ? Number(attrs.max) : 10,
              step: attrs?.step !== undefined ? Number(attrs.step) : 1,
              rangeMode: attrs?.rangeMode === true,
              marks: attrs?.marks !== undefined ? attrs.marks : undefined,
              value: attrs?.value !== undefined ? attrs.value : undefined,
              showTicks: attrs?.showTicks === true,
