...
import NumberComponent from './view';
import { genId } from '../../utils';

export const NumberNode = Node.create({
  name: 'numberField',
  group: 'block',
  atom: false,
  draggable: true,
  ...
  parseHTML() {
    return [{ tag: 'div[data-type="number-field"]' }];
  },

  renderHTML({ HTMLAttributes }) {
    return [
      'div',
      mergeAttributes(HTMLAttributes, { 'data-type': 'number-field' }),
      0,
    ];
  },

  addNodeView() {
    return ReactNodeViewRenderer(NumberComponent);
  },
    ...
    return {
      insertNumberField:
        ...
        ({ commands }: { commands: CommandProps['commands'] }) => {
          const label = (attrs as Attributes)?.label ?? 'Number';
