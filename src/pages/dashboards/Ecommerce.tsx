...
const CategoriesChart = () => {
  ...
  return <Pie {...(config as React.ComponentProps<typeof Pie>)} />;
};
...
const CustomerRateChart = () => {
  ...
  return <Bullet {...(config as React.ComponentProps<typeof Bullet>)} />;
};
...
const OrdersStatusChart = () => {
  ...
  const config = {
    ...
    label: {
      type: 'inner',
      offset: '-30%',
      content: (data: { percent?: number }) =>
        `${((data.percent ?? 0) * 100).toFixed(0)}%`,
      style: {
        fontSize: 14,
        textAlign: 'center',
      },
...
const PRODUCTS_COLUMNS: ColumnsType<ProductRecord> = [
  {
    title: 'Name',
    dataIndex: 'product_name',
    key: 'product_name',
    render: (_: unknown, { product_name, brand }: ProductRecord) => (
...
const SELLER_COLUMNS = [
  {
    ...
    key: 'first_name',
    render: (_: unknown, record: { first_name: string; last_name: string }) => (
  ...
  },
  {
    ...
    key: 'sales_volume',
    render: (_: string) => <span>{numberWithCommas(Number(_))}</span>,
  },
  {
    title: 'Amount',
  ...
  },
  {
    ...
    render: (_: string) => {
      ...
      if (Number(_) < 20) {
        color = red[5];
      } else if (Number(_) > 21 && Number(_) < 50) {
        color = yellow[6];
      } else if (Number(_) > 51 && Number(_) < 70) {
        color = blue[5];
      } else {
        color = green[6];
      }

      return <Progress percent={Number(_)} strokeColor={color} />;
    },
  },
];
...
const ORDERS_COLUMNS = [
  ...
  },
  {
    ...
    key: 'status',
    render: (_: string) => {
      let color: TagProps['color'], icon: React.ReactNode;
