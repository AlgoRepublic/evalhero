...
import { ComprehensiveTagStatsPassFail, MomentumData } from '../../../../services/tagsApi';
...
export const PassFailPanel = ({ passFail, momentum, subjectCount }: PassFailPanelProps) => {
  const { token } = useToken();
  const isMobile = useMediaQuery({ maxWidth: 768 });

  const passFailPercent = passFail.total 
    ? (passFail.passed / passFail.total) * 100 
    : 0;
  const failPercent = passFail.total 
    ? (passFail.failed / passFail.total) * 100 
    : 0;
    ...
    >
      ...
      {momentum && (
        <Text type="secondary" style={{ marginTop: isMobile ? 12 : 16, display: 'block', fontSize: isMobile ? 11 : 12 }}>
          Momentum: {momentum.dPass > 0 ? '+' : ''}
          {momentum.dPass.toFixed(1)}% (last 7d vs prev 7d)
        </Text>
      )}
    </Card>
  );
