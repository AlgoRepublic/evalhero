...
import { useCreateCourseFolderMutation } from '../../../services/coursesApi';
import type { CourseFolder } from '../../../types/course';

export interface CreateCourseFolderModalProps {
  open: boolean;
  onClose: () => void;
...
const CreateCourseFolderModal: React.FC<CreateCourseFolderModalProps> = ({
  open,
  onClose,
  onSuccess,
  parentId,
  ...
  const [createFolder, { isLoading }] = useCreateCourseFolderMutation();
  ...
  const handleOk = async () => {
    try {
      ...
      const folder: CourseFolder | undefined = result?.data?.folder;
...
export default CreateCourseFolderModal;
